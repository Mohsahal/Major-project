...
import { Target, BookOpen, Calendar, AlertCircle, CheckCircle2, Loader2, Upload, FileText, Trash2, Play, Eye, ArrowLeft, Home, PieChart, Activity } from "lucide-react";
...
export default function SkillGapAnalysisPage() {
    ...
    const getSkillPriority = (skillName) => {
        if (!analysisResult)
            return 'Medium';
        const recommendation = analysisResult.learning_resources.recommendations.find(rec => rec.skill.toLowerCase() === skillName.toLowerCase());
    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-8">
          ...
          {/* Analysis Results */}
          {analysisResult && (<div className="space-y-8">
              {/* Summary Cards */}
              ...
              <Tabs defaultValue="overview" className="w-full">
                      ...
                      <CardContent>
                        <div className="flex flex-wrap gap-2">
                          {analysisResult.analysis.present_skills.length > 0 ? (analysisResult.analysis.present_skills.map((skill, index) => (<Badge key={index} variant="outline" className="bg-green-100 text-green-800 text-sm px-3 py-1">
                                ...
                                {skill}
                              </Badge>))) : (<p className="text-gray-500 text-sm">No matching skills found in your resume for this job description.</p>)}
                        </div>
                      </CardContent>
                    </Card>
                      ...
                      <CardContent>
                        <div className="flex flex-wrap gap-2">
                          {analysisResult.analysis.missing_skills.length > 0 ? (analysisResult.analysis.missing_skills.map((skill, index) => (<Badge key={index} variant="outline" className="bg-red-100 text-red-800 text-sm px-3 py-1">
                                ...
                                {skill}
                              </Badge>))) : (<p className="text-gray-500 text-sm">Great! You have all the required skills for this position.</p>)}
                        </div>
                      </CardContent>
                    </Card>
                  </div>
                ...
                <TabsContent value="gaps">
                  {analysisResult.analysis.missing_skills.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {analysisResult.analysis.missing_skills.map((skill, index) => (<Card key={index}>
                          <CardContent className="pt-6">
                            <div className="flex items-center justify-between mb-4">
                              <div className="flex items-center gap-2">
                                <h3 className="font-medium text-lg">{skill}</h3>
                                <Badge variant="outline" className={getSkillPriority(skill) === 'high' ? 'bg-red-100 text-red-800' :
                ...
                <TabsContent value="recommendations">
                  {analysisResult.learning_resources.recommendations.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {analysisResult.learning_resources.recommendations.map((recommendation, index) => (<Card key={index}>
                          <CardContent className="pt-6">
                            <div className="flex items-center justify-between mb-4">
                              <h3 className="font-medium text-lg">{recommendation.skill}</h3>
                              <Badge variant="outline" className={recommendation.priority === 'high' ? 'bg-red-100 text-red-800' :
                        recommendation.priority === 'medium' ? 'bg-yellow-100 text-yellow-800' :
                            ...
                            <div className="space-y-3">
                              {recommendation.resources.map((resource, idx) => (<div key={idx} className="flex items-start gap-2 text-sm">
                ...
                <TabsContent value="videos">
                  {Object.keys(analysisResult.learning_resources.youtube_videos).length > 0 ? (<div className="space-y-8">
                      {Object.entries(analysisResult.learning_resources.youtube_videos).map(([skill, videos]) => (<Card key={skill}>
                          <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                          ...
                          <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                              {videos.map((video, index) => (<div key={index} className="border rounded-lg overflow-hidden bg-white">
                                  ...
                                  <div className="p-4">
                                    ...
                                    <Button size="sm" variant="outline" className="w-full" onClick={() => window.open(video.url, '_blank')}>

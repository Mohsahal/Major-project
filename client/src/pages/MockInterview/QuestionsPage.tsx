...
import { ApiClient, Interview } from "@/config/api";
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (resp.success && resp.data) setInterviews(resp.data);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <Headings
          title="Question Bank"
          description="Browse all generated interview questions from your mock interviews."
        />

        <div className="space-y-6">
          {loading && <p className="text-sm text-muted-foreground">Loading questions…</p>}
          {!loading && interviews.length === 0 && (
            <p className="text-sm text-muted-foreground">No interviews yet. Create one to generate questions.</p>
          )}
            ...
            <Card key={iv.id} className="p-4">
              ...
              <ul className="list-disc pl-5 mt-3 space-y-2">
                ...
                {(!iv.questions || iv.questions.length === 0) && (
                  <li className="text-sm text-muted-foreground">No questions generated for this interview.</li>
                )}
              </ul>
            </Card>
          ))}

...
import { 
  ...
  Shield, Lock, KeyRound, Camera, Upload, Settings, Sparkles, Plus, Trash2, LayoutDashboard, Loader2,
  Languages, Target, Activity, CheckCircle2, Circle, ExternalLink, Clock, TrendingUp
} from "lucide-react";
...
export default function Profile() {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        setIsLoading(true);
        const response = await ApiClient.get<{ success: boolean; data: any }>(API_ENDPOINTS.AUTH.PROFILE);
    ...
    const loadStats = async () => {
      try {
        // Load resume count
        const resumeResponse = await ApiClient.get<any[]>(API_ENDPOINTS.RESUMES);
        const resumeCount = Array.isArray(resumeResponse) ? resumeResponse.length : 0;

        // Load interview count
        const interviewResponse = await ApiClient.get<{ success: boolean; data: any[] }>(API_ENDPOINTS.INTERVIEWS);
        ...
        const recentResumes = Array.isArray(resumeResponse) 
          ? resumeResponse.slice(0, 3).map((r: any) => ({
              type: 'resume',
        ...
        const recentInterviews = interviews.slice(0, 3).map((i: any) => ({
          type: 'interview',
        ...
        setRecentActivity([...recentResumes, ...recentInterviews].sort((a, b) => 
        ...
        setStats({
          resumeCount,
          interviewCount,
          completedInterviews: interviews.filter((i: any) => i.completed).length
        });
      } catch (error) {
  ...
  const addLanguage = () => {
    ...
    if (!value) return;
    if (profileData.languages.includes(value)) return;
  ...
  const addCertification = () => {
    ...
    if (!value) return;
    if (profileData.certifications.includes(value)) return;
  ...
  const achievements = useMemo(() => {
    const ach = [];
    if (profileCompletion >= 100) ach.push({ name: "Complete Profile", icon: Star, color: "text-yellow-500" });
    if (stats.resumeCount >= 1) ach.push({ name: "Resume Creator", icon: FileText, color: "text-blue-500" });
    if (stats.interviewCount >= 5) ach.push({ name: "Interview Pro", icon: Trophy, color: "text-purple-500" });
    if (profileData.skills.length >= 10) ach.push({ name: "Skill Master", icon: Award, color: "text-green-500" });
    return ach;
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid sm:grid-cols-3 gap-2">
                        <Input 
                          ...
                          onChange={(e) => setProfileData({ ...profileData, socials: { ...profileData.socials, github: e.target.value } })}
                        ...
                        <Input 
                          ...
                          onChange={(e) => setProfileData({ ...profileData, socials: { ...profileData.socials, twitter: e.target.value } })}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {isEditing ? (
                            ...
                            <div className="font-medium">{profileData.education || "Not specified"}</div>
                          )}
                        </div>
                      </div>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {isEditing ? (
                            ...
                            <div className="font-medium">{profileData.experience || "Not specified"}</div>
                          )}
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="space-y-3">
                          ...
                          {profileData.skills.map((skill) => (
                            <span key={skill} className="inline-flex items-center gap-2 px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-sm">
                              ...
                              <button 
                                ...
                                className="text-gray-700/70 hover:text-gray-800" 
                        ...
                        <div className="flex gap-2">
                          <Input 
                            ...
                            onKeyDown={(e) => e.key === 'Enter' ? addSkill() : undefined} 
                          />
                      ...
                      <div className="space-y-3">
                        <div className="flex flex-wrap gap-2">
                          {profileData.languages.map((lang) => (
                            <span key={lang} className="inline-flex items-center gap-2 px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-sm">
                              {lang}
                              <button 
                        ...
                        <div className="flex gap-2">
                          <Input 
                            ...
                            onKeyDown={(e) => e.key === 'Enter' ? addLanguage() : undefined} 
                          />
                      ...
                      <div className="flex flex-wrap gap-2">
                        {profileData.languages.length > 0 ? (
                          profileData.languages.map((lang, index) => (
                            <span key={index} className="px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-sm flex items-center gap-1">
              ...
              <Card>
                ...
                <CardContent>
                  {isEditing ? (
                    ...
                    <p className="text-gray-700">{profileData.careerGoals || "No career goals added yet."}</p>
                  )}
                </CardContent>
              </Card>
                    ...
                    <div className="space-y-3">
                      <div className="flex flex-wrap gap-2">
                        {profileData.certifications.map((cert) => (
                          <span key={cert} className="inline-flex items-center gap-2 px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-sm">
                      ...
                      <div className="flex gap-2">
                        <Input 
                          ...
                          onKeyDown={(e) => e.key === 'Enter' ? addCertification() : undefined} 
                        />
                    ...
                    <div className="space-y-4">
                      {recentActivity.map((item, index) => (
                        <div key={index} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50">
                          ...
                          <Link 
                            to={item.type === 'resume' ? `/resume-builder` : `/mock-interview`}
